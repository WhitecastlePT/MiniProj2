...
<script>
import { EDIT_QUIZ, FETCH_QUIZZES } from "@/store/quizzes/quiz.constants";
...
export default {
  ...
  created() {
    // First fetch quizzes to populate the store
    this.$store.dispatch(`quiz/${FETCH_QUIZZES}`).then(
      () => {
        // Then get the specific quiz from the store
  ...
  // Fetch questions again when component becomes active
  activated() {
    this.fetchQuestions();
  }
};
